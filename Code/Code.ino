...
void nextStep(){                                                // Выбираем новую расцветку для сетодиодов
  ...
  for (int e = 0; e < 3; e++) {                                 // Цикл на 3 раза
   ...
   for (int Counter = 0; Counter < 3; Counter++) {              // Задаём каждый из 3 параметров
     LedColor[e][Counter] = colors[i][Counter];                 // Копирование из исходника в массив
   }
  }
}
void DynInd(){                                                  // Обнавляем расцветку сетодиодов
  ...
  for (int Counter = 0; Counter < 3; Counter++) {               // цикл на обнавление 3 светодиодов                       
    analogWrite(RPin, LedColor[Counter][0]);                    // пишем яркость для красного оттенка
    analogWrite(GPin, LedColor[Counter][1]);                    // пишем яркость для зелёного оттенка   
    analogWrite(BPin, LedColor[Counter][2]);                    // пишем яркость для голубого оттенка
